...
def extended_gcd(a, b, p):
    ...
    # return gcd and the modular inverse of a % p
    return old_r, old_s % p, old_t % p

def divide_field(x, y, p):
    gcd, inverse, _ = extended_gcd(y, p, p)  # We use y and p to find the modular inverse
    if gcd != 1:  # If y and p are not coprime, no inverse exists
        raise ValueError("No modular inverse exists for this value")
    return finite_field(x * inverse, p)
